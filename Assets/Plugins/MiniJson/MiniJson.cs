...
public static class Json
{
    ...
    public static object Deserialize(string json)
    {
        ...
        {
            UnityEngine.Debug.LogError("Json - Deserialize fail: " + json);
            return null;
        }
    }
    ...
    private sealed class Parser : IDisposable
    {
        ...
        private object ParseNumber()
        {
            ...
            // This source version
            // if (number.IndexOf('.') == -1) return long.Parse(number, numberFormat); return double.Parse(number, numberFormat);
            ...
            {
                return ulong.Parse(number, numberFormat);
            }
        }
    ...
    private sealed class Serializer
    {
        ...
        private void SerializeOther(object value)
        {
            if (value is float valueFloat)
                builder.Append(valueFloat.ToString(CultureInfo.InvariantCulture));

            else if (value is double valueDouble)
                builder.Append(valueDouble.ToString(CultureInfo.InvariantCulture));

            else if (value is decimal valueDecimal)
                builder.Append(valueDecimal.ToString(CultureInfo.InvariantCulture));

            else if (value is int || value is uint || value is long || value is sbyte || value is byte || value is short || value is ushort || value is ulong || value is decimal)
