...
namespace SG.UI
{
    // https://github.com/DanielKegel/movement_unity/blob/master/Library/PackageCache/com.unity.textmeshpro%401.4.1/Scripts/Runtime/TMP_Dropdown.cs

    [AddComponentMenu("UI/SG/Dropdown", 35)]
    ...
    public class Dropdown : Selectable, IPointerClickHandler, ISubmitHandler, ICancelHandler
    {
        public class DropdownItem : MonoBehaviour, IPointerEnterHandler, ICancelHandler
        {
            ...
            public UnityEngine.UI.Toggle toggle { get { return m_Toggle; } set { m_Toggle = value; } }
        ...
        [Serializable]
        /// <summary>
        /// Class to store the text, sprite and color of a single option in the dropdown list.
        /// </summary>
        ...
        public Action<List<DropdownItem>> onShow;

        private GameObject m_Dropdown;
        private GameObject m_Blocker;
        protected List<DropdownItem> m_Items = new List<DropdownItem>();
        // SG Fix private TweenRunner<FloatTween> m_AlphaTweenRunner;
        ...
        {
            SetValue(UnityEngine.Random.Range(0, options.Count));
        }
        ...
        public void RefreshShownValue()
        {
            ...
            if (m_CaptionImage)
            {
                if (data != null)
                {
                    m_CaptionImage.sprite = data.sprite;
                    if (data.color != null)
                        m_CaptionImage.color = data.color;
                    m_CaptionImage.enabled = data.sprite != null || data.color != null;
                }
                else
                {
        ...
        public void AddOptions(List<string> options)
        {
            for (int i = 0; i < options.Count; i++)
                this.options.Add(new OptionData() { text = options[i] });
        ...
        {
            for (int i = 0; i < options.Count; i++)
                this.options.Add(new OptionData() { sprite = options[i] });
        ...
        private void SetupTemplate()
        {
            ...
            GetOrAddComponent<UnityEngine.UI.GraphicRaycaster>(templateGo);
        ...
        private DropdownItem AddItem(OptionData data, bool selected, DropdownItem itemTemplate, List<DropdownItem> items)
        {
            ...
            if (item.image)
            {
                ...
                item.image.enabled = item.image.sprite != null || data.color != null;
            }
        ...
        public Dropdown SetCallback(Action<int> callback)
        {
