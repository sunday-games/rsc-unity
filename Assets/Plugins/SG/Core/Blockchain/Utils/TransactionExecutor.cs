...
{
    [CreateAssetMenu(menuName = "Sunday/Blockchain/" + nameof(TransactionExecutor), fileName = nameof(TransactionExecutor), order = 0)]
    public class TransactionExecutor : ScriptableObject
    {
        ...
        public void ExecuteButton_OnClick()
        {
            ...
            var transaction = new Transaction(FunctionName);
            transaction.SetFunction(function, content);

            Blockchain.wallet.TransactionSignAndSend(transaction).Start();
        }
    }
}