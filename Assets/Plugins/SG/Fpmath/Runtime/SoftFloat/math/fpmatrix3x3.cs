...
namespace ME.ECS {
    ...
    public static class fpmatrix3x3Ext {
        ...
        public static ref tfloat Get(this ref fpmatrix3x3 m, int row, int column) {
    ...
    public struct fpmatrix3x3 {
        ...
        private static void swap(ref tfloat lhs, ref tfloat rhs) {
        ...
        public bool Invert() {
            ...
            if (math.abs(fDet) > math.VECTOR3_EPSILON) {
                kInverse /= fDet;
                this = kInverse;
                return true;
            } else {
        ...
        public static fpmatrix3x3 operator /(fpmatrix3x3 m, tfloat f) {
            m *= 1.0F / f;
            return m;
        }
        ...
        public void SetAxisAngle(float3 rotationAxis, tfloat radians) {
        ...
        public void MakePositive(float3 euler) {
            tfloat negativeFlip = -0.0001F;
            tfloat positiveFlip = math.PI * 2.0F - 0.0001F;
        ...
        public void EulerToMatrix(float3 v, fpmatrix3x3 matrix) {
            tfloat cx = math.cos(v.x);
            tfloat sx = math.sin(v.x);
            tfloat cy = math.cos(v.y);
            tfloat sy = math.sin(v.y);
            tfloat cz = math.cos(v.z);
            tfloat sz = math.sin(v.z);
        ...
        private void GetRotMatrixNormVec(ref fpmatrix3x3 data, float3 inVec, tfloat radians) {
            /* This function contributed by Erich Boleyn (erich@uruk.org) */
            /* This function used from the Mesa OpenGL code (matrix.c)  */
            tfloat s, c;
            tfloat vx, vy, vz, xx, yy, zz, xy, yz, zx, xs, ys, zs, one_c;
