...
namespace ME.ECSEditor.FPMath {
    ...
    internal static class Helper {
        ...
        public static sfloat GetFPValue(SerializedProperty property) {
            ...
            return sfloat.FromRaw(val.uintValue);
        ...
        public static void SetFPValue(SerializedProperty property, sfloat value) {
            ...
            var val = property.FindPropertyRelative("rawValue");
            val.uintValue = value.RawValue;
            
        }

        private static UnityEditor.SerializedProperty activeProperty;
        public static void BuildContextMenu(Rect position, UnityEditor.SerializedProperty property)
        {
            ...
            if (e.type == EventType.MouseDown && e.button == 1 && position.Contains(e.mousePosition)) {
                ...
                context.AddItem(new GUIContent ("FromFPToSFloat"), property.isExpanded, () =>
                {
                    activeProperty.serializedObject.Update();
                    activeProperty.FindPropertyRelative("rawValue").uintValue = FromFPToSFloat(activeProperty.FindPropertyRelative("rawValue").uintValue).RawValue;
