...
namespace SG
{
    public class IAPFacebookCanvas : IAPStore
    {
        ...
        {
            if (string.IsNullOrEmpty(iap.facebookUrl))
            {
...
#if FACEBOOK
            FB.Canvas.Pay(product: server.links.hosting + iap.facebookUrl, callback: PurchaseCallback);
#endif
        }

#if FACEBOOK
        void PurchaseCallback(IPayResult result)
        {
